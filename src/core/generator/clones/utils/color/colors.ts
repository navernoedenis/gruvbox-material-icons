...
export const replacementMap = (
  ...
  const isSpecific = isFolder && baseColor.endsWith('Base');
  const isColorful = isFolder && baseColor.endsWith('Dim');
  ...
  if (isSpecific) {
    const [colorname_] = baseColor.split('Base');
    for (let index = 0; index < orderedColors.length; index++) {
      const colorname = index === 0 ? 'base' : colorname_;
      replacement.set(orderedColors[index], colorname);
    }
    return replacement;
  }

  if (isColorful) {
    const [colorname_] = baseColor.split('Dim');
    for (let index = 0; index < orderedColors.length; index++) {
      const colorname = index === 0 ? colorname_ : 'white';
      replacement.set(orderedColors[index], colorname);
    }
    return replacement;
  }
